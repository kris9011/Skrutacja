...
import { BreviaryHourType, BreviaryAudience, ScrutationSession } from '../types';
...
export const BreviaryView: React.FC<BreviaryViewProps> = ({
  ...
  const [activeHour, setActiveHour] = useState<BreviaryHourType>(() => {
    // Pick hour automatically based on current local hour of day if not explicitly passed
  ...
  const [isChantingSoundPlaying, setIsChantingSoundPlaying] = useState<boolean>(false);
  ...
  // Web Audio Bell chime for entering prayer silence
  const playSacredChime = () => {
    try {
      ...
      // Synthesize gentle deep monastery bell harmonics
      ...
      setIsChantingSoundPlaying(true);
      setTimeout(() => setIsChantingSoundPlaying(false), 2600);
    } catch (e) {
  ...
  const fontClasses = [
    ...
    'text-xl leading-[2.2rem]'   // 2: Large
  ][fontSizeLevel];
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-900 pb-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-sans font-bold bg-amber-400/20 text-amber-300 border border-amber-400/40 uppercase tracking-widest">
                    {audience === 'lay' ? 'Dla Świeckich' : 'Dla Duchownych i Zakonów'}
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2 self-end md:self-center">
              ...
              <button
                ...
                title="Dzwonek modlitewny / Gong na wejście w ciszę"
                className={`p-2 rounded-xl bg-white/10 hover:bg-white/15 text-amber-300 border border-amber-500/30 transition-all cursor-pointer active:scale-95 ${
                  isChantingSoundPlaying ? 'ring-2 ring-amber-400 bg-amber-500/20' : ''
                }`}
              >
              ...
              <div className="flex items-center rounded-xl bg-white/10 border border-amber-500/30 p-0.5">
                ...
                <span className="text-[10px] text-amber-400 font-mono px-1">
                  {fontSizeLevel === 0 ? '1x' : fontSizeLevel === 1 ? '1.2x' : '1.4x'}
                </span>
                <button
                  type="button"
            ...
            {audience === 'lay' ? (
              ...
              <>
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-lg text-xs font-sans font-bold whitespace-nowrap transition-all cursor-pointer ${
                    activeHour === 'tercia'
                      ? 'bg-amber-400 text-slate-950 font-extrabold shadow-sm'
                      : 'bg-white/5 text-amber-200/80 hover:bg-white/10 hover:text-white'
                  }`}
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <article className="bg-[#FFFDF9] rounded-3xl border border-amber-900/15 shadow-xl p-6 sm:p-12 space-y-10 font-serif">
          ...
          <div className="text-center space-y-2 border-b-2 border-amber-800/20 pb-8">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-100/70 border border-amber-300/60 text-amber-900 text-xs font-sans font-medium mt-2">
              <Clock className="w-3.5 h-3.5 text-amber-700" />
              <span>Zalecana pora: {currentHourData.recommendedTime}</span>
            </div>
          </div>
          ...
          <section className="space-y-8 border-b border-amber-200/60 pb-10">
            ...
            {currentHourData.psalmody.map((psalm, idx) => (
              <div key={psalm.id || idx} className="space-y-4 p-5 sm:p-7 rounded-2xl bg-[#FCFAF5] border border-amber-900/10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-amber-200/60 pb-3">
                  ...
                  {onStartScrutationWithVerse && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-800 text-emerald-50 hover:bg-emerald-700 text-xs font-sans font-bold transition-all shadow-xs cursor-pointer self-start sm:self-center"
                      title="Rozpocznij skrutację tego Psalmu"
                    >
                      <Scroll className="w-3.5 h-3.5 text-amber-300" />
                      <span>Skrutuj Psalm</span>
                    </button>
                  )}
                </div>

                {/* Antiphon Start */}
                <div className="p-3 rounded-xl bg-amber-100/60 border-l-4 border-amber-600 text-amber-950 text-xs sm:text-sm font-sans font-semibold">
                  <span className="text-rose-800 mr-2 uppercase font-bold text-[10px]">Antyfona:</span>
                  {psalm.antiphon}
                </div>

                {/* Psalm Text */}
                <div className={`space-y-3 ${fontClasses} text-slate-800 pl-2 whitespace-pre-line leading-relaxed`}>
                  {psalm.text}
                </div>
          ...
          <section className="space-y-4 border-b border-amber-200/60 pb-8">
            ...
            {/* Quick Action: Start Scrutatio with Reading */}
            {onStartScrutationWithVerse && (
          ...
          {/* 5. Evangelical Canticle (Benedictus / Magnificat / Nunc Dimittis) */}
          {currentHourData.evangelicalCanticle && (
            <section className="space-y-4 border-b border-amber-200/60 pb-8 p-6 rounded-2xl bg-gradient-to-br from-amber-100/40 via-amber-50/60 to-white border border-amber-300/80 shadow-xs">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <span className="text-[11px] font-sans font-bold uppercase tracking-wider text-amber-900 flex items-center gap-1.5">
                  ...
                  Kantyk Ewangeliczny • {currentHourData.evangelicalCanticle.name}
                </span>
